...
def _doctor() -> int:
    ...
    if architecture in {"x86_64", "amd64"}:
        print("Kodak-Treiberziel: geeignet (Intel/AMD 64 Bit)")
    else:
        print("Kodak-Treiberziel: eingeschränkt; x86_64-VM empfohlen")

    for binary in ("lsusb", "scanimage"):
    ...
    if code not in (0, 127) and usb_text:
        print(f"lsusb-Hinweis: {usb_text}")
    ...
    if kodak_usb and not kodak_sane:
        print("\nNächster Schritt: Kodak-x86_64-Treiber installieren und danach 'scanimage -L' erneut prüfen.")
    if not kodak_usb:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    subparsers.add_parser("doctor", help="VM, USB, SANE und Treiber prüfen")
