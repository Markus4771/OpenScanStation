...
def _page(notice: str = "", error: bool = False, result: dict | None = None) -> str:
    ...
    test = '''<section class="panel"><h2>Erkennung testen</h2><form method="post" action="/classify"><label>Dateiname<input name="filename" value="test.pdf"></label><label>OCR-Text<textarea name="text" placeholder="Hier OCR-Text einfügen"></textarea></label><label>Tags<input name="tags" placeholder="Eingang, Kunde"></label><label>Barcode-/QR-Inhalte<input name="barcodes" placeholder="INV:12345, https://..."></label><button>Dokument klassifizieren</button></form></section>'''
    ...
    info = '<section class="panel"><h2>Phase 3</h2><p>Die Klassifizierung nutzt OCR-Text, Dateiname, Tags und Barcode-/QR-Inhalte. Die Regeln sind nachvollziehbar priorisiert. Ohne Treffer wird der Typ <code>dokument</code> verwendet.</p><p class="muted">Regeln: /var/lib/openscanstation/classification_rules.json · Ergebnisse: /var/lib/openscanstation/classification_results.json</p></section>'
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif path == "/health": self._send(json.dumps({"status":"ok","service":"openscanstation-classification","version":VERSION,"port":PORT}), ctype="application/json")
