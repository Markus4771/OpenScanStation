...
def _module_health(port: int) -> bool:
    conn = None
    try:
        conn = http.client.HTTPConnection("127.0.0.1", port, timeout=2); conn.request("GET", "/health"); response = conn.getresponse(); response.read(); return 200 <= response.status < 300
...
def _processing_page() -> bytes:
    ...
    for name, port, href, description in modules:
        online = _module_health(port); cards.append(f'<article class="oss-processing-card"><span class="oss-status-pill {"oss-ok" if online else "oss-bad"}">{"Bereit" if online else "Nicht erreichbar"}</span><h2>{name}</h2><p>{description}</p><a href="{href}">Öffnen</a></article>')
...
def _rewrite_html(body: bytes, prefix: str) -> bytes:
    ...
    shell = _sidebar()
    if "<body" in text:
        text = re.sub(r"(<body[^>]*>)", lambda m: m.group(1) + shell, text, count=1, flags=re.I); text = re.sub(r"</body>", "</div></body>", text, count=1, flags=re.I)
    else: text = STYLE + shell + text + "</div>"
    return text.encode()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _send(self, payload: bytes, status: int = 200, ctype: str = "text/html; charset=utf-8") -> None:
        self.send_response(status); self.send_header("Content-Type", ctype); self.send_header("Content-Length", str(len(payload))); self.send_header("X-OpenScanStation-Gateway", "1"); self.end_headers(); self.wfile.write(payload)
    def _proxy(self) -> None:
        parsed = urlsplit(self.path)
        if parsed.path in {"/processing", "/processing/"}: self._send(_processing_page()); return
        if parsed.path == "/api/gateway/modules": self._send(json.dumps({"storage":_module_health(8103),"workflows":_module_health(8104),"classification":_module_health(8105),"hardware":_module_health(8107)}).encode(), ctype="application/json"); return
        port, backend_path, prefix = _route(parsed.path); target = backend_path + (("?" + parsed.query) if parsed.query else ""); length = int(self.headers.get("Content-Length", "0") or "0"); body = self.rfile.read(length) if length else None
        headers = {k:v for k,v in self.headers.items() if k.lower() not in HOP_HEADERS and k.lower() != "host"}; headers["Host"] = f"127.0.0.1:{port}"; conn = None
        try:
            conn = http.client.HTTPConnection("127.0.0.1", port, timeout=300); conn.request(self.command, target, body=body, headers=headers); response = conn.getresponse(); payload = response.read()
            if "text/html" in response.getheader("Content-Type", ""): payload = _rewrite_html(payload, prefix)
            self.send_response(response.status, response.reason)
            for key,value in response.getheaders():
                lower=key.lower()
                if lower in HOP_HEADERS or lower=="content-length": continue
                if lower=="location" and prefix and value.startswith("/"): value=prefix+value
                self.send_header(key,value)
            self.send_header("Content-Length",str(len(payload))); self.send_header("X-OpenScanStation-Gateway","1"); self.end_headers(); self.wfile.write(payload)
        except Exception as exc: self._send(f"Modul auf internem Port {port} nicht erreichbar: {exc}".encode(),502,"text/plain; charset=utf-8")
        finally:
            if conn is not None: conn.close()
    do_GET=_proxy; do_POST=_proxy; do_PUT=_proxy; do_DELETE=_proxy; do_PATCH=_proxy
    def log_message(self, fmt: str, *args) -> None: print(fmt % args)


def main(argv=None) -> int:
    parser=argparse.ArgumentParser(); parser.add_argument("--host",default=HOST); parser.add_argument("--port",type=int,default=PORT); args=parser.parse_args(argv); server=ThreadingHTTPServer((args.host,args.port),Handler); print(f"OpenScanStation Gateway läuft auf {args.host}:{args.port}")
