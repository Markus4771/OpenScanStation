...
SCRIPT=r"""<script>(function(){const k='oss.menu.v3',ds=[...document.querySelectorAll('details[data-menu]')];let s={};try{s=JSON.parse(localStorage.getItem(k)||'{}')}catch(e){}ds.forEach(d=>{if(d.dataset.menu in s)d.open=!!s[d.dataset.menu];d.addEventListener('toggle',()=>{s[d.dataset.menu]=d.open;localStorage.setItem(k,JSON.stringify(s))})});const p=location.pathname.replace(/\/$/,'')||'/';document.querySelectorAll('.oss-menu a').forEach(a=>{const u=new URL(a.href,location.href),h=u.pathname.replace(/\/$/,'')||'/',hm=u.hash?u.hash===location.hash:!location.hash;if(hm&&(h==='/'?p==='/':h==='/hardware'?p===h:p===h||p.startsWith(h+'/'))){a.classList.add('active');let d=a.closest('details');while(d){d.open=true;d=d.parentElement.closest('details')}}});const toggle=document.querySelector('.oss-menu-toggle'),side=document.querySelector('.oss-sidebar');if(toggle&&side)toggle.addEventListener('click',()=>side.classList.toggle('menu-open'))})();</script>"""
STYLE="""<style>:root{--w:280px}body{margin:0!important;background:#f6f8fb!important}.oss-sidebar{position:fixed;inset:0 auto 0 0;width:var(--w);box-sizing:border-box;background:#0d2135;color:#fff;padding:22px 14px;overflow:auto;z-index:9999}.oss-brand-row{display:flex;align-items:center;justify-content:space-between}.oss-brand{font-size:25px;font-weight:800;padding:0 10px 18px}.oss-brand small{font-size:12px;color:#a9bbca}.oss-menu-toggle{display:none;background:#165cae;color:#fff;border:0;border-radius:8px;padding:9px 12px;font-weight:800}.oss-menu{display:grid;gap:5px}.oss-menu a,.oss-menu summary{display:block;padding:10px 13px;border-radius:8px;color:#fff!important;text-decoration:none!important;font-weight:700;cursor:pointer}.oss-menu a:hover,.oss-menu a.active,.oss-menu summary:hover{background:#165cae}.oss-menu details>div{display:grid;margin-left:16px;border-left:1px solid #496075;padding-left:8px}.oss-menu details details{margin:3px 0}.oss-menu details details>summary{font-size:.9rem;color:#c9d5df!important}.oss-menu details details>div{margin-left:10px}.oss-menu .oss-logout{margin-top:12px;color:#c9d5df!important}.oss-content-shell{margin-left:var(--w);min-height:100vh}.oss-content-shell main{max-width:1500px!important;margin:0 auto!important;padding:34px!important}.oss-content-shell>header,.oss-content-shell>nav{display:none!important}@media(max-width:700px){.oss-sidebar{position:relative;width:100%;max-height:none}.oss-brand{padding-bottom:0}.oss-menu-toggle{display:block}.oss-sidebar:not(.menu-open) .oss-menu{display:none}.oss-content-shell{margin-left:0}.oss-content-shell main{padding:18px!important}}</style>"""
def sidebar(is_admin: bool = True):
 admin = '<details data-menu="admin"><summary>Verwaltung</summary><div><a href="/users">Benutzer</a><a href="/system">System</a></div></details>' if is_admin else ''
 return f'''<aside class="oss-sidebar"><div class="oss-brand-row"><div class="oss-brand">OpenScanStation<br><small>Version {VERSION}</small></div><button class="oss-menu-toggle" type="button" aria-label="Navigation öffnen">Menü</button></div><nav class="oss-menu"><a href="/">Dashboard</a><a href="/documents">Dokumente</a><details data-menu="scan" open><summary>Scannen</summary><div><a href="/my-scanners">Meine Scanner</a><a href="/profiles">Scanprofile</a><a href="/scanner-actions">Scanneraktionen</a></div></details><details data-menu="processing" open><summary>Verarbeitung</summary><div><a href="/processing">Übersicht</a><a href="/storage/">Speicherziele</a><a href="/workflows/">Workflows</a><a href="/classification/">Dokumenterkennung</a><a href="/copy/">Kopieren</a></div></details><details data-menu="hardware" open><summary>Hardware</summary><div><a href="/hardware/">Übersicht</a><a href="/hardware/scanners">Scanner verwalten</a><a href="/hardware/brother-buttons">Brother-Tasten</a><a href="/storage/#brother-network">Brother To Network</a><details data-menu="hardware-tools"><summary>Weitere Werkzeuge</summary><div><a href="/hardware/device-center">Gerätezentrale</a><a href="/hardware/monitor">Monitor</a><a href="/hardware/setup">Assistent</a><a href="/hardware/profile-assignment">Profilzuordnung</a><a href="/hardware/printers">Drucker</a><a href="/hardware/network">Netzwerk</a><a href="/hardware/usb">USB-Geräte</a><a href="/hardware/brother">Brother-Assistent</a><a href="/hardware/brother-profiles">Brother-Geräteprofile</a><a href="/hardware/maintenance">Wartung</a><a href="/hardware/drivers">Treiber</a><a href="/hardware/diagnostics">Diagnose</a><a href="/hardware/support">Supportpaket</a></div></details></div></details>{admin}<a class="oss-logout" href="/logout">Abmelden</a></nav></aside><div class="oss-content-shell">{SCRIPT}'''
def route(path):
...
def rewrite(body,prefix,is_admin=True):
 ...
 text=re.sub(r"(<body[^>]*>)",lambda m:m.group(1)+sidebar(is_admin),text,count=1,flags=re.I)
...
class Handler(BaseHTTPRequestHandler):
 ...
 def profile_page(self,message="",error=False):
  ...
  self.send(rewrite(render(message,error,user.get("username",""),user.get("role")=="admin").encode(),"",user.get("role")=="admin"),400 if error else 200)
 ...
 def users_page(self,user,message=""):
  ...
  self.send(rewrite(content.encode(),"",True))
 def proxy(self):
  ...
  if p.path=="/my-scanners":
   ...
   return self.send(rewrite(page.encode(),"",user.get("role")=="admin"))
  ...
  try:
   ...
   if 'text/html' in ctype:data=rewrite(data,prefix,user.get("role")=="admin")
