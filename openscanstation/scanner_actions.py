    ...
    if isinstance(actions, list):
        ...
        for raw in actions:
            ...
            item.update({
                ...
                "workflow": str(raw.get("workflow", "standard"))[:64],
                "post_action": str(raw.get("post_action", "none"))[:64],
            })
    bindings = data.get("bindings")
    if isinstance(bindings, dict):
        result["bindings"] = {str(scanner_id)[:512]: {str(key)[:64]: str(value)[:64] for key, value in mapping.items() if isinstance(mapping, dict)} for scanner_id, mapping in bindings.items()}
    return result


def load_actions() -> dict:
    try: return _normalize(json.loads(ACTIONS_FILE.read_text(encoding="utf-8")))
    except (FileNotFoundError, json.JSONDecodeError, OSError): return deepcopy(DEFAULT_ACTIONS)


def save_actions(data: dict) -> dict:
    normalized = _normalize(data); ACTIONS_FILE.parent.mkdir(parents=True, exist_ok=True)
    fd, temp_name = tempfile.mkstemp(prefix="scanner-actions-", suffix=".json", dir=ACTIONS_FILE.parent)
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as handle:
            json.dump(normalized, handle, ensure_ascii=False, indent=2); handle.write("\n"); handle.flush(); os.fsync(handle.fileno())
...
def action_for_event(scanner_id: str, event: str) -> dict | None:
    data = load_actions(); mapping = data["bindings"].get(scanner_id, {}); action_id = mapping.get(event)
    if not action_id and event.startswith("slot-"): action_id = f"action-{event.removeprefix('slot-')}"
...
def set_binding(scanner_id: str, event: str, action_id: str) -> dict:
    data = load_actions(); data["bindings"].setdefault(scanner_id, {})[event] = action_id
