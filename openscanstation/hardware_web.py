...
class Handler(BaseHTTPRequestHandler):
    ...
    def _get(self, head_only: bool = False) -> None:
        ...
        if path == "/health":
            payload = json.dumps({
                "status": "ok",
                "service": "openscanstation-hardware",
                "version": VERSION,
                "architecture": "modular",
            })
            self.send(payload, ctype="application/json", head_only=head_only)
            return
        ...
        if path == "/support/download":
            ...
            self.send(bundle.read_bytes(), ctype="application/gzip",
                      disposition=f'attachment; filename="{bundle.name}"',
                      head_only=head_only)
            return
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if path == "/scanner/test":
                ...
                self.send(layout(f"<section class='panel'><h2>Verbindungstest</h2><pre>{json.dumps(result, ensure_ascii=False, indent=2)}</pre><a class='button' href='/scanners'>Zurück</a></section>", "Scanner-Test"))
                return
            ...
            if path == "/scanner/test-scan":
                ...
                self.send(layout(f"<section class='panel'><h2>Testscan erfolgreich</h2><pre>{json.dumps(result, ensure_ascii=False, indent=2)}</pre><a class='button' href='/scanners'>Zurück</a></section>", "Testscan", result.get("message", "")))
                return

            if path == "/scanner/manual-add":
                from openscanstation.hardware_management import add_manual_scanner
                add_manual_scanner(get("name"), get("uri"), get("backend"))
                self.send(render("/setup") or "")
                return

            if path == "/scanner/manual-delete":
                from openscanstation.hardware_management import delete_manual_scanner
                delete_manual_scanner(get("scanner_id"))
                self.send(render("/scanners") or "")
                return
            ...
            if path == "/printer/add":
                ...
                self.send(layout("<section class='panel'><h2>Drucker eingerichtet</h2><a class='button' href='/printers'>Drucker öffnen</a></section>", "Drucker", result.get("message", "")))
                return

            if path == "/printer/test":
                from openscanstation.hardware import print_test_page
                result = print_test_page(get("printer"))
                self.send(layout("<section class='panel'><h2>Testseite</h2><a class='button' href='/printers'>Zurück</a></section>", "Drucker", result.get("message", "")))
                return

            if path == "/printer/cancel":
                from openscanstation.hardware import cancel_print_job
                cancel_print_job(get("job"))
                self.send(render("/printers") or "")
                return

            if path == "/network/probe":
                from openscanstation.hardware import tcp_probe
                result = tcp_probe(get("host"))
                self.send(layout(f"<section class='panel'><h2>Netzwerktest</h2><pre>{json.dumps(result, ensure_ascii=False, indent=2)}</pre><a class='button' href='/network'>Zurück</a></section>", "Netzwerktest"))
                return

            if path == "/maintenance/save":
                from openscanstation.hardware_management import save_maintenance
                save_maintenance(get("device_id"), task=get("task"), interval_days=int(get("interval_days", "90")), last_done=get("last_done"))
                self.send(render("/maintenance") or "")
                return

            if path == "/maintenance/complete":
                from openscanstation.hardware_management import complete_maintenance
                complete_maintenance(get("item_id"))
                self.send(render("/maintenance") or "")
                return
            ...
            if path == "/support/create":
                ...
                self.send(layout("<section class='panel'><h2>Supportpaket erstellt</h2><a class='button' href='/support/download'>Herunterladen</a></section>", "Support", result.get("message", "")))
                return

            self.send(json.dumps({"error": "not_found", "path": path}), 404, "application/json")
        except Exception as exc:
            self.send(layout("<section class='panel'><h2>Aktion fehlgeschlagen</h2><a class='button' href='/'>Hardware-Übersicht</a></section>", "Hardware", str(exc), True), HTTPStatus.BAD_REQUEST)
...
class HardwareServer(ThreadingHTTPServer):
    ...
    def handle_error(self, request, client_address) -> None:
        # Browserabbrüche sollen nicht als lange Tracebacks im Journal erscheinen.
        import sys
        exc = sys.exc_info()[1]
