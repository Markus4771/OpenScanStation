...
STYLE = """
body{font-family:system-ui;margin:0;background:#f3f6f9;color:#17202a}header{background:#17202a;color:#fff;padding:1.2rem 2rem}header a{color:#fff;text-decoration:none;margin-right:1rem}main{max-width:1450px;margin:1.4rem auto;padding:0 1rem}.panel,.card{background:#fff;padding:1.2rem;border-radius:14px;box-shadow:0 2px 14px #0001;margin-bottom:1rem}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:1rem}.metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:1rem}.metric{font-size:2rem;font-weight:800}.status{display:inline-block;padding:.25rem .6rem;border-radius:999px;font-weight:700}.ok{background:#d5f5e3;color:#196f3d}.bad{background:#fadbd8;color:#922b21}.warn{background:#fcf3cf;color:#7d6608}.muted{color:#687684}.actions{display:flex;gap:.55rem;flex-wrap:wrap;align-items:center}a.button,button{display:inline-block;background:#17202a;color:#fff;padding:.7rem 1rem;border:0;border-radius:8px;text-decoration:none;font-weight:700;cursor:pointer}a.secondary,button.secondary{background:#e9eef3;color:#17202a}input,select{box-sizing:border-box;padding:.7rem;border:1px solid #bcc5cc;border-radius:8px;width:100%}form{display:grid;gap:.65rem}form.inline{display:inline-block}table{width:100%;border-collapse:collapse}th,td{padding:.7rem;border-bottom:1px solid #e1e5e8;text-align:left;vertical-align:top}pre{white-space:pre-wrap;overflow:auto;max-height:480px;background:#f6f7f8;padding:1rem;border-radius:8px}code{overflow-wrap:anywhere}.notice{padding:1rem;border-radius:9px;margin-bottom:1rem}.success{background:#d5f5e3}.error{background:#fadbd8}.step{border-left:4px solid #1769d2;padding-left:1rem}.hero{display:flex;justify-content:space-between;gap:1rem;align-items:center;flex-wrap:wrap}.small{font-size:.9rem}@media(max-width:700px){header{padding:1rem}main{padding:.5rem}table{display:block;overflow-x:auto}}
"""
...
def device_card(device: dict) -> str:
    flags = ''.join(f'<span class="status ok">{esc(k)}</span> ' for k,v in device.get('capabilities',{}).items() if v is True)
    href = f'/{device["kind"]}/{quote(str(device["id"]), safe="")}'
    return f'<article class="card"><div class="actions"><span class="status {"ok" if device.get("online") else "bad"}">{"Online" if device.get("online") else "Offline"}</span>{"<span class=\"status ok\">Standard</span>" if device.get("default") else ""}{"<span class=\"status warn\">Deaktiviert</span>" if not device.get("enabled",True) else ""}</div><h2>{esc(device["name"])}</h2><p><b>{esc(device["kind"].title())}</b> · {esc(device.get("manufacturer",""))} {esc(device.get("model",""))}</p><p><code>{esc(device.get("connection","-"))}</code></p><p>{flags or "Keine erweiterten Fähigkeiten gemeldet"}</p><a class="button" href="{href}">Gerät öffnen</a></article>'


def overview(notice: str = "", error: bool = False) -> str:
    data = inventory(); c = data['counts']; drivers = driver_status(); events = hardware_events(8)
    cards = ''.join(device_card(d) for d in data['devices']) or '<article class="card"><h2>Keine Geräte gefunden</h2><p>Starte den Hardware-Assistenten.</p><a class="button" href="/setup">Assistent öffnen</a></article>'
    ...
    driver_ok = sum(1 for v in drivers.values() if v)
    content = f'<section class="panel hero"><div><h2>Hardware-Dashboard</h2><p class="muted">Scanner, Drucker und Schnittstellen zentral verwalten.</p></div><div class="actions"><a class="button" href="/setup">Hardware hinzufügen</a><form class="inline" method="post" action="/refresh"><button class="secondary">Neu suchen</button></form></div></section><div class="metrics"><article class="card"><b>Scanner</b><div class="metric">{c["scanner"]}</div></article><article class="card"><b>Drucker</b><div class="metric">{c["printer"]}</div></article><article class="card"><b>Online</b><div class="metric">{c["online"]}</div></article><article class="card"><b>Druckaufträge</b><div class="metric">{c.get("print_jobs",0)}</div></article><article class="card"><b>Treiber bereit</b><div class="metric">{driver_ok}/{len(drivers)}</div></article></div><div class="grid">{cards}</div><section class="panel"><h2>Letzte Ereignisse</h2><table><tr><th>Zeit</th><th>Typ</th><th>Meldung</th></tr>{event_rows}</table></section>'
    return layout(content, notice=notice, error=error)


def setup_page() -> str:
    brother = brother_assistant(); net = network_discovery(); usb = usb_devices(); inv = inventory()
    status = 'ok' if brother['scanner_found'] else 'warn'
    brother_text = 'Brother-Scanner erkannt' if brother['scanner_found'] else 'Noch kein Brother-Scanner erkannt'
    net_rows = ''.join(f'<tr><td>{esc(d.get("name",""))}</td><td>{esc(d.get("protocol",""))}</td><td><code>{esc(d.get("uri",""))}</code></td></tr>' for d in net[:15]) or '<tr><td colspan="3">Keine Netzwerkgeräte gefunden.</td></tr>'
    content = f'<section class="panel"><h2>Hardware-Assistent</h2><p>Der Assistent prüft Scanner, Drucker, Treiber, Netzwerk und USB.</p></section><div class="grid"><article class="card step"><h2>1. Erkennung</h2><p>{len(inv["devices"])} eingerichtete Geräte, {len(net)} Netzwerkfunde, {len(usb)} USB-Geräte.</p><form method="post" action="/refresh"><button>Erkennung wiederholen</button></form></article><article class="card step"><h2>2. Brother ADS-2600We</h2><span class="status {status}">{brother_text}</span><p>SANE: {"bereit" if brother["sane"] else "fehlt"} · AirScan: {"bereit" if brother["airscan"] else "fehlt"} · Brother-Treiber: {"vorhanden" if brother["driver"] else "nicht erkannt"}</p><a class="button" href="/brother">Brother-Assistent</a></article><article class="card step"><h2>3. Drucker</h2><p>CUPS-Drucker können unter Drucker verwaltet und getestet werden.</p><a class="button" href="/printers">Drucker öffnen</a></article></div><section class="panel"><h2>Gefundene Netzwerkgeräte</h2><table><tr><th>Name</th><th>Protokoll</th><th>Adresse</th></tr>{net_rows}</table></section>'
...
def scanners(notice: str = "", error: bool = False, result: dict | None = None) -> str:
    ...
    return layout(f'<section class="panel hero"><div><h2>Scanner</h2><p class="muted">Erkennen, benennen, testen und als Standard festlegen.</p></div><a class="button" href="/brother">Brother-Assistent</a></section>{result_box}<div class="grid">{"".join(blocks) or "<article class=card><h2>Kein Scanner gefunden</h2><a class=button href=/setup>Assistent öffnen</a></article>"}</div>', 'Scanner', notice, error)
...
def printers(notice: str = "", error: bool = False) -> str:
    ...
    for d in devices:
        pid=d['id']; alias=settings.get('printer_aliases',{}).get(pid,''); jobs=''.join(f'<li>{esc(j.get("raw",""))} <form class="inline" method="post" action="/printer/cancel"><input type="hidden" name="job" value="{esc(j.get("id",""),True)}"><button class="secondary">Abbrechen</button></form></li>' for j in d.get('jobs',[])) or '<li>Keine offenen Aufträge</li>'
        blocks.append(f'<article class="card"><h2>{esc(d["name"])}</h2><span class="status {"ok" if d.get("online") else "bad"}">{"Online" if d.get("online") else "Offline"}</span><p><code>{esc(d.get("connection",""))}</code></p><form method="post" action="/printer/save"><input type="hidden" name="printer" value="{esc(pid,True)}"><label>Anzeigename<input name="alias" value="{esc(alias,True)}"></label><label>Aktiv<select name="enabled"><option value="1" {"selected" if d.get("enabled") else ""}>Ja</option><option value="0" {"selected" if not d.get("enabled") else ""}>Nein</option></select></label><label>Standard<select name="make_default"><option value="0">Nein</option><option value="1" {"selected" if d.get("default") else ""}>Ja</option></select></label><button>Speichern</button></form><div class="actions"><form class="inline" method="post" action="/printer/test"><input type="hidden" name="printer" value="{esc(pid,True)}"><button class="secondary">Testseite</button></form><a class="button secondary" href="/printer/{quote(pid,safe="")}">Details</a></div><details><summary>Druckaufträge</summary><ul>{jobs}</ul></details></article>')
    return layout(f'<section class="panel hero"><div><h2>Drucker</h2><p class="muted">CUPS-Drucker verwalten, testen und Warteschlangen bearbeiten.</p></div><a class="button" href="/network">Netzwerkdrucker suchen</a></section><div class="grid">{"".join(blocks) or "<article class=card><h2>Kein Drucker eingerichtet</h2><p>Prüfe CUPS und die Netzwerkerkennung.</p></article>"}</div>', 'Drucker', notice, error)


def detail(kind: str, device_id: str) -> str:
    d=next((x for x in inventory()['devices'] if x['kind']==kind and x['id']==device_id),None)
    if not d: return layout('<section class="panel"><h2>Gerät nicht gefunden</h2></section>', 'Hardware', 'Gerät nicht gefunden', True)
    rows=''.join(f'<tr><th>{esc(k)}</th><td><pre>{esc(json.dumps(v,ensure_ascii=False,indent=2) if isinstance(v,(dict,list)) else v)}</pre></td></tr>' for k,v in d.items())
    return layout(f'<section class="panel"><h2>{esc(d["name"])}</h2><table>{rows}</table></section>', d['name'])
...
def brother_page() -> str:
    data=brother_assistant(); scanners=''.join(device_card(d) for d in data['scanners']) or '<p>Kein Brother-Scanner in SANE erkannt.</p>'; air=''.join(f'<li>{esc(d.get("name",""))}: <code>{esc(d.get("uri",""))}</code></li>' for d in data['airscan_devices']) or '<li>Kein Brother-AirScan-Gerät gefunden.</li>'
    rec=''.join(f'<li>{esc(x)}</li>' for x in data['recommendations'])
    return layout(f'<section class="panel"><h2>Brother ADS-2600We Assistent</h2><div class="actions"><span class="status {"ok" if data["sane"] else "bad"}">SANE</span><span class="status {"ok" if data["airscan"] else "bad"}">AirScan</span><span class="status {"ok" if data["driver"] else "warn"}">Brother-Treiber</span></div><ol>{rec}</ol></section><div class="grid">{scanners}</div><section class="panel"><h2>AirScan-Funde</h2><ul>{air}</ul><p>Beim ADS-2600We ist die treiberlose Netzwerkverbindung über AirScan/eSCL normalerweise die bevorzugte Variante. Der Assistent verändert keine Firmware.</p></section>', 'Brother-Assistent')
...
def drivers_page() -> str:
    rows=''.join(f'<tr><td>{esc(k)}</td><td><span class="status {"ok" if v else "bad"}">{"Bereit" if v else "Fehlt"}</span></td></tr>' for k,v in driver_status().items())
...
def diagnostics_page() -> str:
    blocks=''.join(f'<section class="panel"><h2>{esc(k)}</h2><span class="status {"ok" if v.get("ok",True) else "bad"}">{"OK" if v.get("ok",True) else "Fehler"}</span><pre>{esc(json.dumps(v,ensure_ascii=False,indent=2))}</pre></section>' for k,v in diagnostics().items())
    return layout(blocks, 'Diagnose')


class Handler(BaseHTTPRequestHandler):
    def send(self, data: str | bytes, status: int = 200, ctype: str = 'text/html; charset=utf-8') -> None:
        body=data.encode() if isinstance(data,str) else data; self.send_response(status); self.send_header('Content-Type',ctype); self.send_header('Content-Length',str(len(body))); self.send_header('X-Content-Type-Options','nosniff'); self.end_headers(); self.wfile.write(body)

    def do_GET(self) -> None:
        path=urlparse(self.path).path
        if path=='/': self.send(overview())
        elif path=='/setup': self.send(setup_page())
        elif path=='/scanners': self.send(scanners())
        elif path=='/printers': self.send(printers())
        elif path=='/network': self.send(network_page())
        elif path=='/usb': self.send(usb_page())
        elif path=='/brother': self.send(brother_page())
        elif path=='/maintenance': self.send(maintenance())
        elif path=='/drivers': self.send(drivers_page())
        elif path=='/diagnostics': self.send(diagnostics_page())
    ...
    def log_message(self, fmt: str, *args) -> None: print(fmt % args)
